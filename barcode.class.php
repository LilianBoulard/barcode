...
class PluginBarcodeBarcode {
   ...
   /**
   * @param string    $qrcode_file    Requis. Chemin du fichier image QRcode.
   *
   * @return array                    Retourne les valeurs du QRcode, sous la forme d'un tableau associatif (un dictionnaire).
   */
   static function getQRcodeInformation($qrcode_file) {
      # Passe le chemin vers le fichier image QRcode au lecteur (php-qrcode-detector-decoder library) ; changer entre "Zxing\QrReader" et "QrReader" si l'un des deux ne fonctionne pas.
      $qrcode = new Zxing\QrReader($qrcode_file);
      # Récupère les informations du QRcode
      $text = $qrcode->text();
      # Remplace les espaces par des underscores pour les valeurs avec plusieurs mots
      $text = str_replace("Serial Number", "Serial_Number", $text);
      $text = str_replace("QRcode date", "QRcode_date", $text);
      # Divise la ligne en utilisant les espaces comme des délimiteurs
      $array = preg_split('/[\s]+/', $text);
      foreach ($array as $k) {
         # Si la valeur est un signe égal
         if (($key = array_search("=", $array)) !== FALSE) {
            # On supprime la valeur du tableau
            unset($array[$key]);
            # Et on réindexe le tableau
            $array = array_values(array_filter($array));
         }
      }
      # Initialise le tableau associatif
      $dict = [];
      while (1) { # Boucle infinie
         if ($array[0]) { # Si $array[0] existe
            # Ajoute une valeur (paire clé<->valeur) au tableau associatif
            $dict[$array[0]] = $array[1];
            # On retire les deux valeurs du tableau
            unset($array[0]);
            unset($array[1]);
            # Et on le réindexe
            $array = array_values(array_filter($array));
         } else { # Si $array[0] n'existe pas
            # Supprimer le tableau de la mémoire
            unset($array);
   ...
   function printPDF($p_params) {
      ...
      foreach ($codes as $code) {
         ...
         if ($code != '') {
            ...
            if (file_exists($imgFile)) {
               ...
               # Créer une nouvelle classe barcode
               $barcode = new self();
               # Il faut passer comme argument à la fonction suivante la variable contenant le chemin du fichier QRcode
               $qrCodeValues = $barcode->getQRcodeInformation($p_params['codes'][0]);
               # Place le curseur sous le QRcode
               $pdf->ezSetY($y + 4);
               # Ajoute le nom de l'ordinateur à l'emplacement du pointeur
               $pdf->ezText($qrCodeValues['Name'], $txtSize, array('aleft'=>50));

               if ($p_params['border']) {
                  $pdf->Rectangle($x, $y - 13, $width, $height + 16);
               }
            }
         }
         $x += $width + $marginH;
      ...
      $file    = $pdf->ezOutput();
      $pdfFile = $qrCodeValues['Name'].'_'.$type.'.pdf';
